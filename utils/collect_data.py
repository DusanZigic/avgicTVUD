...
class collectData:
	...
	def collect_urqmd(self, hydrojobid):
		work_dir = path.abspath("work")
	...
	def collect_analysis(self):
		...
		for job_id in range(len(job_dirs)):
			...
			job_dir = job_dirs[job_id]
			centrality = self.params['main']['centrality'][job_id].replace('-', '').replace('%', '')

			for aFile in self.params['analysis']['save_files']:
				rename(path.join(job_dir, aFile), path.join(dest_dir, aFile.replace(".dat", f"{centrality}.dat")))

			rmtree(job_dir)

	def collect_dreena(self, job_id):
		work_dir = path.abspath("work")
