...
energyLoss::energyLoss(int argc, const char *argv[])
{
	...
	if ((inputs.size() == 1) && (inputs[0] == "-h")) {
		std::cout << "default values: --srcDirectory=./ --collsys=PbPb --sNN=5020GeV --pName=Charm --xB=0.6 --xGridN=50 --yGridN=50 --phiGridN=25 --TIMESTEP=0.1 --TCRIT=0.155" << std::endl;
		m_error = true;
	}
	...
	std::vector<std::string> arguments = {"srcDirectory", "collsys", "sNN", "pName", "xB", "xGridN", "yGridN", "phiGridN", "TIMESTEP", "TCRIT", "config", "h"};
	for (const auto &inputParam : inputParams) {
		if(std::find(arguments.begin(), arguments.end(), inputParam.first) == arguments.end()) {
			...
			std::cerr << "--srcDirectory=./ --collsys=PbPb --sNN=5020GeV --pName=Charm --xB=0.6 --xGridN=50 --yGridN=50 --phiGridN=25 --TIMESTEP=0.1 --TCRIT=0.155" << std::endl;
	...
	std::vector<std::string> argumentsFile = {"srcDirectory", "collsys", "sNN", "pName", "xB", "xGridN", "yGridN", "phiGridN", "TIMESTEP", "TCRIT"};
